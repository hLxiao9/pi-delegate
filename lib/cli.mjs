...
export const COMMANDS = ['init', 'doctor', 'detect', 'prepare', 'run', 'revise', 'verify', 'self-review', 'recover', 'approve', 'integrate', 'report', 'cleanup', 'list', 'inspect', 'dashboard', 'serve'];
...
Usage:
  ...
  pi-worker detect
  pi-worker prepare --task <task.json> [--profile <name>] [--model <id>] [--select-worker true] [--select-model true] [--usage-start <usage-start.json>]
...
export async function main(argv, io = process, handlers = {}) {
  ...
  // Resolve caller the same way prepare records state.caller:explicit PARENT_AGENT /
  // PI_WORKER_CALLER, then a WorkBuddy shell marker, else 'cli'. WorkBuddy-launched
  // sessions that do not set PARENT_AGENT are still attributed to 'workbuddy'.
  const caller = (process.env.PARENT_AGENT ?? process.env.PI_WORKER_CALLER ?? (process.env.WORKBUDDY ?? process.env.WORKBUDDY_AGENT ? 'workbuddy' : 'cli'));
