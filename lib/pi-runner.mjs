...
import { assertDelegableCapabilities } from './contracts.mjs';
...
export function buildPiPrompt(task, mode, evidence = null) {
  const safeTask = { ...task, repositoryRoot: '.' };
  const common = [
...
export async function runPiTurn({ paths, files, state, task, config, env, mode, evidence = null }) {
  ...
  throw new WorkerError('PI_FAILED', 'Pi implementation turn failed', {
    category: finalAttempt.failure, exitCode: finalAttempt.result.code, timedOut: finalAttempt.result.timedOut, stderr: finalAttempt.result.stderr,
  });
}
