...
function finalizeInvoke({ adapter, result, lastAssistant, rawStdout, rawStderr, invalidJsonLines, attempt, phase, profile, parsedUsage, files }) {
  ...
  const failure = ok ? null : (invalidJsonLines > 0 ? 'permanent' : adapter.classifyFailure({ stderr: rawStderr, stdout: rawStdout, exitCode: result.code, timedOut: result.timedOut }));
  ...
  // usage:Pi 从 lastAssistant.usage 取;Kimi/Trae 从 parsedUsage 取(可能为 null)
  const usage = lastAssistant?.usage ?? parsedUsage ?? null;
...
export async function runPiTurn({ paths, files, state, task, config, env, mode, evidence = null, promptOverride = null, expectJsonOutput = false }) {
  ...
  throw new WorkerError('PI_FAILED', `${adapter.name} implementation turn failed`, {
...
export async function reviseCommand(options = {}, runtime = {}) {
  ...
  return withRunLock(paths, options.id, async () => {
    ...
    try {
      ...
      const isPiFailure = error.code === 'PI_FAILED' && error.details?.category !== 'auth';
      if (isPiFailure) {
