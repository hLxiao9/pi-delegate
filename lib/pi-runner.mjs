...
// Convert a single worker stdout line (NDJSON event for streaming adapters, or
// a line of one-shot text for OpenCode) into a compact, human-readable entry
// for worker.log. Returns null when there is nothing worth showing (blank or
// unparseable noise). This is the readable companion to the raw pi-events.jsonl
// so a human can `tail -f` a run without parsing JSON.
function formatLogLine(rawLine) {
  const line = String(rawLine || '').trim();
  if (!line) return null;
  let event = null;
  try { event = JSON.parse(line); } catch { /* not JSON -> show as-is */ }
  ...
  switch (type) {
  case 'text':
    return typeof event?.part?.text === 'string' ? event.part.text : null;
  case 'tool_call': {
    const name = event?.part?.name ?? event?.part?.tool ?? 'tool';
    const args = event?.part?.arguments ?? event?.part?.input;
    const summary = typeof args === 'string' ? args : (args ? JSON.stringify(args) : '');
    return `→ ${name}${summary ? ` ${summary.slice(0, 200)}` : ''}`;
  }
  case 'tool_result':
    return `← result ${String(event?.part?.output ?? '').slice(0, 200)}`;
  case 'step_start':
    return `--- step: ${event?.part?.description ?? event?.part?.text ?? ''} ---`.slice(0, 200);
  case 'step_finish': {
    const t = event?.part?.tokens;
    return t ? `=== tokens: in=${t.input ?? 0} out=${t.output ?? 0} total=${t.total ?? 0} ===` : null;
  }
  case 'result':
...
async function invokeWorker({ paths, files, state, profile, env, prompt, config, attempt, phase }) {
  ...
  let rawStdout = '';
  let rawStderr = '';
  let rawStdoutTruncated = false;
  const rawStdoutLimit = config.limits.maxCapturedCharsPerStream;

  // Human-readable run banner at the top of worker.log so a `tail -f` session
  // clearly shows which run/profile/model/phase this stream belongs to.
  appendFileSync(files.workerLog,
  `[${new Date().toISOString()}] run=${state.runId} profile=${profile.name} adapter=${adapter.name} model=${profile.model || '(default)'} phase=${phase} attempt=${attempt}\n`,
  { mode: 0o600 });
  ...
  onStdoutLine(line) {
  // Bound rawStdout to prevent OOM on long runs (issue #1 P2-7).
  // process.mjs already caps its own `stdout`, but rawStdout here is
  // a separate accumulation used for parseOutput fallback.
  if (!rawStdoutTruncated && rawStdout.length + line.length + 1 > rawStdoutLimit) {
  rawStdoutTruncated = true;
  } else if (!rawStdoutTruncated) {
  rawStdout += line + '\n';
  }
  if (line.length === 0) return;
  appendFileSync(files.events, `${line}\n`, { mode: 0o600 });
  const readable = formatLogLine(line);
  if (readable) appendFileSync(files.workerLog, `${readable}\n`, { mode: 0o600 });
  try {
  ...
  // OpenCode also uses this branch (--format json is one-shot NDJSON). We still
  // tee each line into worker.log as human-readable text so a human can follow
  // the run live via `tail -f` without parsing JSON.
  const result = await runProcess(bin, argv, {
  cwd: cwd ?? state.worktreePath,
  env: workerEnv,
  input,
...
function finalizeInvoke({ adapter, result, lastAssistant, rawStdout, rawStderr, invalidJsonLines, attempt, phase, profile, parsedUsage, files }) {
  ...
  // Adapter-agnostic usage capture:Pi streaming already emits `message_end`
  // events with `usage` into the events file, which readPiUsage() later parses
  // into metrics. Non-streaming adapters (OpenCode/Kimi/Trae/Qoder) only
  // produce a usage object here in finalizeInvoke, so unless we echo it back
  // into the events file as a message_end event, readPiUsage() returns zeros
  // and the dashboard shows no tokens/duration for those connectors. We mirror
  // the exact shape readPiUsage expects so every adapter feeds the same path.
  if (usage && !adapter.supportsStreaming) {
  appendFileSync(files.events, `${JSON.stringify({
  type: 'message_end',
  message: { role: 'assistant', usage: { input: usage.inputTokens, output: usage.outputTokens, cacheRead: usage.cachedInputTokens, cacheWrite: usage.cacheWriteInputTokens, reasoning: usage.reasoningOutputTokens, totalTokens: usage.totalTokens, cost: { total: usage.providerReportedCostUsd } } },
  })}\n`, { mode: 0o600 });
  }
...
export async function runPiTurn({ paths, files, state, task, config, env, mode, evidence = null, promptOverride = null, expectJsonOutput = false }) {
  ...
  // A model chosen at prepare/run time (--model or the interactive picker)
  // overrides the profile's pinned model, enabling per-task model selection
  // without editing config.json. Empty/auto values fall through to the
  // adapter's own default model.
  const effectiveModel = (state.model || '').trim();
  if (effectiveModel && effectiveModel !== 'auto' && effectiveModel !== 'default') {
    profile = { ...profile, model: effectiveModel };
  }
  const adapter = resolveAdapter(profile);
  ...
  if (adapter.name !== 'trae' && profile.apiKeyEnv && !env[profile.apiKeyEnv]) {
  ...
  // Fallback chain: on a transient failure (rate-limit / quota exhausted /
  // network), walk the ordered fallbackProfiles list and retry each in turn.
  // This realizes "auto-switch to the next (free) model when one runs out".
  // A visited set prevents cycles; self-managed workers (no apiKeyEnv, e.g.
  // opencode) skip the credential check.
  if (finalAttempt.failure === 'transient') {
  const visited = new Set([profile.name]);
  let current = profile;
  for (const candidateName of profile.fallbackProfiles) {
    if (visited.has(candidateName)) continue;
    visited.add(candidateName);
    const fallback = selectProfile(config, candidateName);
    if (!fallback) continue;
    assertDelegableCapabilities(task, fallback);
    const fallbackAdapter = resolveAdapter(fallback);
    if (fallbackAdapter.name !== 'trae' && fallback.apiKeyEnv && !env[fallback.apiKeyEnv]) {
    continue; // cannot authenticate this fallback -> try the next one
    }
    ...
    current = fallback;
    if (finalAttempt.failure !== 'transient') break; // non-retryable -> stop
  }
  }
...
export async function runCommand(options = {}, runtime = {}) {
  ...
  if (options.model && options.model.trim() && options.model.trim() !== 'auto') {
    state = { ...state, model: options.model.trim() };
  }
  const result = await runPiTurn({ paths, files: latest.files, state, task: latest.task, config, env, mode: 'run' });
