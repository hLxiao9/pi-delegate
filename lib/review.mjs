...
export async function approveCommand(options = {}, runtime = {}) {
  ...
  return withRunLock(paths, options.id, async () => {
    ...
    // selfReviewing 状态下 approve = 主控跳过 self-review 直接 review;先转到 reviewing 再走 approve 流程。
    ...
    try {
      ...
      // commit 失败时保持 approved 状态(可重试),而非转 failed(不可恢复)。
      // recoverCommand 只接受 PI_INTERRUPTED/PI_TIMEOUT 的 failed,COMMIT_FAILED 会永久 bricked。
      // commit 失败通常是 hooks/lock/I/O 临时问题,保持 approved 让用户可重试 approve。
