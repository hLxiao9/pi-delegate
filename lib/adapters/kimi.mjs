...
export const KimiAdapter = {
  ...
  // Kimi non-interactive mode uses `-p <prompt>` (verified against kimi v0.29.1).
  // Kimi has NO stdin-prompt support — neither `kimi -` (unknown command) nor
  // `-p -` (treats '-' as a literal prompt) read from stdin — so the prompt MUST
  // be passed as the `-p` argv value. This is a known cmdline-leak limitation
  // specific to Kimi; other secrets stay off argv via selectedEnvironment.
  // Tool permissions are controlled by config.toml [permission.rules], not a flag.
  const argv = ['-p', prompt];
