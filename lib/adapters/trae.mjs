...
export const TraeAdapter = {
  ...
  // traecli reads prompt from stdin (append '-') to avoid argv leakage
  // (ps / /proc/<pid>/cmdline). Output via --json.
  ...
  // stdin marker must be last: some CLI parsers stop processing flags after
  // the first positional argument, and '-' is conventionally a positional.
  argv.push('-');
