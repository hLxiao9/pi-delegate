...
export const WorkerAdapter = {
  ...
  classifyFailure({ stderr, stdout, exitCode, timedOut }) {
    ...
    // transient: retryable infrastructure / quota-exhausted conditions.
    // quota exhaustion is treated as transient so the runner can fall back to
    // the next profile (e.g. a free model) instead of failing the task.
    if (/\b(408|409|425|429|500|502|503|504)\b|rate.?limit|econnreset|etimedout|temporar|overloaded|network|quota|insufficient.{0,24}(balance|quota|credit|fund)|exceeded|out.?of.{0,12}(credit|quota|token)|billing/.test(text)) return 'transient';
    return 'permanent';
  },
};
