...
export async function verifyCommand(options = {}, runtime = {}) {
  ...
  return withRunLock(paths, options.id, async () => {
    ...
    const commands = [];
    for (const specification of loaded.task.verification) {
      ...
      commands.push({ argv: specification.argv, timeoutSeconds: specification.timeoutSeconds, code: result.code, signal: result.signal, timedOut: result.timedOut, durationMs: result.durationMs, stdout: result.stdout, stderr: result.stderr, stdoutTruncated: result.stdoutTruncated, stderrTruncated: result.stderrTruncated, passed: result.code === 0 && !result.timedOut });
    }
    const passed = commands.every((command) => command.passed);
    const evidence = { schemaVersion: 1, runId: options.id, passed, security, commands, at: new Date().toISOString() };
    await writeJsonAtomic(loaded.files.verification, evidence);
    const updated = await updateRun(paths, options.id, (state) => transition(state, 'reviewing', { lastVerificationPassed: passed }));
