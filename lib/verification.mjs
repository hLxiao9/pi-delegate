...
export async function verifyCommand(options = {}, runtime = {}) {
  ...
  return withRunLock(paths, options.id, async () => {
    ...
    if (outputTruncated) {
      // 输出截断:安全 fail-closed。截断可能意味着验证命令输出异常(攻击或配置问题),
      // 转 blocked(终态)比保持 verifying 更安全。用户可手动调整配置后用 recover 恢复。
    ...
    // 验证未通过时不进入 selfReviewing(self-review 会因 verification.passed=false 卡死)。
