...
export async function prepareCommand(options = {}, runtime = {}) {
  ...
  // 并发限制:统计当前处于活跃状态(running/verifying/reviewing/revising)的 run 数,
  // 超过 maxConcurrentRuns 则拒绝 prepare,防止多会话并发时耗光 API 配额。
  const activeStates = (await listRunStates(paths)).filter((s) => ['running', 'verifying', 'reviewing', 'revising'].includes(s.status));
  ...
  // 用户传 --profile 则尊重;否则按难度+领域+模态自动路由。
  ...
  // M6: worktree 创建后若 snapshot 失败,清理已创建的 worktree + branch,避免残留。
