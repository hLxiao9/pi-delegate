...
import { snapshotCodexUsage } from './codex-usage.mjs';
...
export async function prepareCommand(options = {}, runtime = {}) {
  ...
  let codexUsageStart;
  let codexUsageStartSource;
  if (options['codex-start']) {
    codexUsageStart = await readJson(path.resolve(options['codex-start']));
    invariant(typeof codexUsageStart?.available === 'boolean' && typeof codexUsageStart.at === 'string' && Number.isFinite(Date.parse(codexUsageStart.at)), 'CONTRACT_INVALID', 'The Codex usage baseline is invalid');
    codexUsageStartSource = 'early-meter';
  } else {
    codexUsageStart = await snapshotCodexUsage({ home: env.HOME, threadId: env.CODEX_THREAD_ID });
    codexUsageStartSource = 'prepare-fallback';
  }
