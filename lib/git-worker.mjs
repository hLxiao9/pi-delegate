...
// Resolve the effective model id for a run, applying three sources of intent
// (in priority order) so the user never has to hand-edit model ids in config:
//   1. explicit `--model <id>` override            -> no prompting
//   2. the profile's pinned `model` (config.json)  -> used as-is
//   3. interactive picker when a human is at a TTY  -> lists models via the
//      adapter's `listModels` and lets them choose; skipped when a parent agent
//      (Codex/MyBard) drives pi-delegate headlessly (would otherwise hang)
//   4. empty string -> the adapter/worker uses its own default (zero-config)
async function resolveModelForPrepare({ requestedModel, forceSelect, profile, adapter, paths, env }) {
  if (requestedModel && requestedModel.trim() && requestedModel.trim() !== 'auto') {
    return requestedModel.trim();
  }
  const pinned = (profile.model || '').trim();
  if (!forceSelect && pinned && pinned !== 'auto') {
    return pinned;
  }
  // Interactive only when a human is present; parent agents drive headlessly.
  const interactive = Boolean(forceSelect)
    || (Boolean(process.stdin.isTTY) && !process.env.PARENT_AGENT && !process.env.PI_WORKER_CALLER);
  if (!interactive || !adapter.supportsModelList) {
    return pinned; // no picker available -> fall back to pinned or empty (default)
  }
  let models = [];
  try {
    const spec = await adapter.listModels({ paths, profile, env, bin: 'opencode' });
    if (spec && Array.isArray(spec.argv)) {
      const bin = resolveBin(adapter, env);
      const out = await runProcess(bin, spec.argv, { env: spec.env, timeoutMs: 30000 });
      if (out.code === 0 && spec.parse) models = spec.parse(out.stdout);
    }
  } catch {
    models = [];
  }
  if (!models.length) return pinned;
  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  try {
    process.stdout.write('\nOpenCode available models:\n');
    models.forEach((m, i) => process.stdout.write(`  ${String(i + 1).padStart(2)}. ${m.id}\n`));
    const ans = (await rl.question(`Select a model [1-${models.length}] (Enter = OpenCode default): `)).trim();
    if (!ans) return '';
    const idx = Number.parseInt(ans, 10);
    if (Number.isFinite(idx) && idx >= 1 && idx <= models.length) return models[idx - 1].id;
    return '';
  } finally {
    rl.close();
  }
...
export async function prepareCommand(options = {}, runtime = {}) {
  ...
  const routed = options.profile ? null : selectProfileForTask(configured, task);
  ...
  const profileName = routed?.name ?? options.profile;
  ...
  // Resolve the model for this run: explicit --model > profile pin > interactive
  // TTY picker > adapter default. Stored on state so run/revise reuse the choice
  // without re-prompting and without editing config.json.
  const effectiveModel = await resolveModelForPrepare({
    requestedModel: options.model,
    forceSelect: options['select-model'] === 'true' || options.interactive === 'true',
    profile,
    adapter: preAdapter,
    paths,
    env,
