...
function callerBadgeClass(caller) {
  ...
  if (caller === 'workbuddy') return 'badge blue';
...
function renderSummaryCards(summary) {
  ...
  <div class="card"><div class="card-label">call count</div><div class="card-value">${summary.totalPiRequests}</div><div class="card-sub">duration ${formatDuration(summary.totalPiDurationMs)}</div></div>
  <div class="card"><div class="card-label">token total</div><div class="card-value">${formatTokens(summary.totalPiInput + summary.totalPiOutput)}</div><div class="card-sub">in ${formatTokens(summary.totalPiInput)} / out ${formatTokens(summary.totalPiOutput)}</div></div>
  ...
  <div class="card"><div class="card-label">Equivalent parent-side credits</div><div class="card-value">${summary.totalEquivalentCredits.toFixed(2)}</div><div class="card-sub">tokens converted at parent rate card</div></div>
...
// loadShellProfileEnv has been extracted to ./shell-profile.mjs so that doctor,
// pi-runner, server, dashboard, and prepare all share one implementation.
// The dashboard still calls it (via buildConnectionsPayload below) to merge
// shell-profile credentials as a fallback when the server process env does not
// inherit them (e.g., GUI launch, non-login shell).
...
function renderFilters() {
  ...
  <select id="filter-caller"><option value="">All sources</option><option value="workbuddy">workbuddy</option><option value="trae">trae</option><option value="codex">codex</option><option value="claude-code">claude-code</option><option value="cursor">cursor</option><option value="pi-recursive">pi-recursive</option><option value="cli">cli</option><option value="unknown">unknown</option></select>
...
export function renderDashboardFragment(states, metricsList, generatedAt, options = {}) {
  ...
  return {
  generatedAt,
  bodyHtml: renderSummaryCards(summary) + '\n' + renderFilters() + '\n' + (empty || '<table>\n  <thead><tr><th>Run ID</th><th>Created at</th><th>Source</th><th>status</th><th>model</th><th>Token</th><th>duration</th><th>Equivalent credits</th><th>Allowance share</th><th>saving rate</th><th>verification</th><th>balance</th><th>Commit</th></tr></thead>\n  <tbody>' + rows.map(renderRow).join('') + '</tbody>\n</table>') + '\n' + aggregation,
  };
}

// Per-panel toolbar (Refresh button + auto-refresh toggle). Rendered at the top
// of each live tab-panel so users can refresh without a full browser reload.
// The auto-refresh toggle drives a 60s poll (see bindAutoRefresh in the script).
function panelToolbarHtml() {
  return '<div class="panel-toolbar">' +
  '<button class="panel-refresh-btn" type="button">Refresh</button>' +
  '<label class="auto-refresh-toggle"><input type="checkbox" id="auto-refresh-checkbox" /> Auto-refresh (60s)</label>' +
  '<span class="auto-refresh-status" id="auto-refresh-status"></span>' +
  '</div>';
}
...
function dashboardScript(options) {
  ...
  const refreshJs = '  let dashboardRefreshing = false;\n' +
  ...
  '  try { bindCopyButtons(); } catch (err) { console.warn("bindCopyButtons failed", err); }\n' +
  '  if (status) status.textContent = "updated " + formatLocal(fragData.generatedAt);\n' +
  '  } catch (err) {\n' +
  ...
  '  refreshBtn.addEventListener("click", function () { refreshDashboard(); });\n' +
  '  });\n' +
  '  }\n' +
  ...
  '  if (!checkbox) return;\n' +
  '  let enabled = false;\n' +
  '  try { enabled = localStorage.getItem("pi-worker-autorefresh") === "1"; } catch (e) {}\n' +
  '  checkbox.checked = enabled;\n' +
  '  function startTimer() {\n' +
  '  if (dashboardRefreshTimer) return;\n' +
  '  dashboardRefreshTimer = setInterval(function () { if (document.visibilityState === "visible") refreshDashboard(); }, 60000);\n' +
  '  }\n' +
  '  function stopTimer() { if (dashboardRefreshTimer) { clearInterval(dashboardRefreshTimer); dashboardRefreshTimer = null; } }\n' +
  '  if (enabled) startTimer();\n' +
  '  checkbox.addEventListener("change", function () {\n' +
  '  const on = checkbox.checked;\n' +
  '  try { localStorage.setItem("pi-worker-autorefresh", on ? "1" : "0"); } catch (e) {}\n' +
  '  if (on) { startTimer(); refreshDashboard(); } else { stopTimer(); }\n' +
  '  });\n' +
  '  // Refresh once when the tab becomes visible again after being hidden,\n' +
  '  // so returning to the dashboard shows current state without waiting a minute.\n' +
  '  document.addEventListener("visibilitychange", function () {\n' +
  '  if (document.visibilityState === "visible" && checkbox.checked) refreshDashboard();\n' +
  '  });\n' +
  '  window.addEventListener("focus", function () {\n' +
  '  if (checkbox.checked) refreshDashboard();\n' +
  '  });\n' +
  '  }\n' +
  '  try { bindRefreshButtons(); } catch (err) { console.warn("bindRefreshButtons failed", err); }\n' +
  '  try { bindAutoRefresh(); } catch (err) { console.warn("bindAutoRefresh failed", err); }';
  return '<script>\n' + bindEventsJs + '\n' + initJs + '\n' + refreshJs + '\n</script>';
}
...
export function renderDashboardHtml(states, metricsList, generatedAt, options = {}) {
  ...
  const panelToolbar = live ? panelToolbarHtml() : '';
  ...
  '  .auto-refresh-toggle { display: inline-flex; align-items: center; gap: 6px; font-size: 13px; color: var(--muted); cursor: pointer; user-select: none; }',
  '  .auto-refresh-toggle input { cursor: pointer; }',
  '  .auto-refresh-status { font-size: 12px; color: var(--muted); margin-left: 4px; }',
  ] : []),
