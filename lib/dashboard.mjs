...
// 从用户 shell profile (~/.zshrc, ~/.bash_profile 等) 解析 export 语句,
// 返回指定环境变量的值。用于 dashboard 凭证状态检测的兜底:
// 当 server 进程 env 缺少变量时(例如从 GUI/非交互 shell 启动),
// 尝试从用户 shell profile 读取,避免误报"未配置"。
// 只返回请求的变量名,不泄露其他变量;不记录 key 值到日志。
...
async function loadShellProfileEnv(envVarNames, options = {}) {
  ...
  for (const file of SHELL_PROFILE_FILES) {
    ...
    for (const line of content.split('\n')) {
      // 匹配 export NAME='value' / export NAME="value" / export NAME=value
...
export async function buildConnectionsPayload(runtime = {}) {
  ...
  // 兜底:合并用户 shell profile 里的 env。process.env 优先(运行时注入的值更可信),
  // shell profile 作为兜底(解决 server 从 GUI/非交互 shell 启动时缺少 API key 的问题)。
...
export function renderDashboardHtml(states, metricsList, generatedAt, options = {}) {
  ...
  const panelToolbar = live ? '<div class="panel-toolbar"><button class="panel-refresh-btn" type="button">\u5237\u65b0</button></div>' : '';
  const dashboardPanel = live
    ? '<div id="dashboard-body">\n' + fragment.bodyHtml + '\n</div>'
    : fragment.bodyHtml;
  const connectionsPanel = '<div id="connections-body">\n' + connectionsHtml + '\n</div>';
  const footerNote = live ? '\u70b9\u51fb\u9762\u677f\u4e0a\u65b9"\u5237\u65b0"\u6309\u94ae\u62c9\u53d6\u6700\u65b0 run \u6570\u636e\u3002' : '\u7531 <code>pi-worker dashboard</code> \u751f\u6210\u3002\u91cd\u65b0\u8fd0\u884c\u4ee5\u5237\u65b0\u3002';
  // CSS for the restructured header layout:
  // - Row 1: title + generated timestamp (header .titles)
  // - Row 2: menubar with tab buttons (left) and theme toggle (right)
  ...
  const css = [
    ...
    '  header .titles { padding-bottom: 12px; }',
  ...
  const html = [
    ...
    '  </div>',
    '  <nav class="menubar" aria-label="\u4e3b\u9009\u5355">',
    '    <div class="tabs">',
