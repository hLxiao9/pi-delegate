...
// Stricter match for forbidden paths: in addition to matchesGlob, also check
// each path segment against the pattern's keyword. This catches cases like
// `secrets/backup.txt` where the directory name contains "secret" but the
// filename does not, which `**/*secret*` would miss (issue #1 P2-10).
function matchesForbidden(relative, patterns) {
  if (matches(relative, patterns)) return true;
  const segments = relative.split('/');
  for (const pattern of patterns) {
  // Extract keyword from patterns like **/*secret* or **/*credential*
  const keywordMatch = pattern.match(/\*\*\/\*(\w+)\*/);
  if (keywordMatch) {
  const keyword = keywordMatch[1].toLowerCase();
  if (segments.some((seg) => seg.toLowerCase().includes(keyword))) return true;
  }
  // Also check bare extension patterns like .env.*
  if (pattern.startsWith('.env') && segments.some((seg) => seg.startsWith('.env'))) return true;
  }
  return false;
}
...
export async function scanWorkerDelta({ paths, state, task, config, env = process.env }) {
  // `git add -N` (intent-to-add) lets untracked files appear in `git diff`
  // output so the security scan can see new files Pi created.
  // Note (issue #1 P2-22): this does leave intent-to-add markers in the index,
  // but this is benign - subsequent verify/approve flows always re-run
  // `git add -A` which supersedes the -N entries. We do NOT clean them up
  // because removing them would make newly-created files invisible to
  // later diff operations within the same run.
