...
export async function scanWorkerDelta({ paths, state, task, config, env = process.env }) {
  ...
  const namesResult = await git(paths, state.worktreePath, ['diff', '--name-only', '-z', '--no-renames', state.workerBaseRevision], { env, maxCaptureChars: limit });
  ...
  const numstatResult = await git(paths, state.worktreePath, ['diff', '--numstat', '-z', '--no-renames', state.workerBaseRevision], { env, maxCaptureChars: limit });
  ...
  const diffResult = await git(paths, state.worktreePath, ['diff', '--unified=0', '--no-ext-diff', '--no-renames', state.workerBaseRevision], { env, maxCaptureChars: limit });
  issues.push(...secretIssues(diffResult.stdout));
  if (Buffer.byteLength(diffResult.stdout) > config.limits.maxDiffBytes) issues.push({ code: 'DIFF_TOO_LARGE', message: 'Worker diff exceeds configured maximum size' });
