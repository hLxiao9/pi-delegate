...
import { detectEnvironment, resolveInstalledBin } from './environment.mjs';
...
async function doctor(options = {}, runtime = {}) {
  ...
  // Resolve the actual binary, probing candidateBins (e.g. opencode's
  // ~/.opencode/bin) when the primary resolved bin isn't on PATH.
  const bin = await resolveInstalledBin(adapter, env);
  ...
  // On missing credential, fall back to the user's shell profile (~/.zshrc etc.)
  // so a non-login shell (e.g., GUI launch) is not falsely flagged as unconfigured.
  // The shared buildCredentialHint also surfaces the 'name mismatch' trap (user wrote
  // API_KEY but config expects VOLCENGINE_API_KEY) and the 'found in shell profile but
  // not inherited by this process' trap (start from a login shell or source profile).
  ...
  // Worker manages its own credentials (e.g. opencode reads its own provider
  // config via `opencode providers login`). No env var required.
  checks.push({ name: 'credential', ok: true, type: 'self-managed', hint: 'OpenCode manages its own provider credentials; no env var required.' });
  } else {
  if (!env[profile.apiKeyEnv]) {
  let message = await buildCredentialHint(profile.apiKeyEnv, { home: env.HOME });
  // Fix 6: when the default profile's credential is missing, scan all other
  // profiles and suggest --profile if any of them has its credential configured
  // (either in process.env or in the user's shell profile).
  const otherProfileEntries = Object.entries(config.profiles || {})
  .filter(([name]) => name !== profile.name)
  .map(([name, p]) => p && typeof p === 'object' && typeof p.apiKeyEnv === 'string' ? { name, apiKeyEnv: p.apiKeyEnv } : null)
  .filter((entry) => entry !== null);
  if (otherProfileEntries.length > 0) {
  const otherEnvNames = otherProfileEntries.map((p) => p.apiKeyEnv);
  const otherProfileEnv = await loadShellProfileEnv(otherEnvNames, { home: env.HOME });
  const mergedOther = { ...otherProfileEnv, ...env };
  const available = otherProfileEntries.find((p) => Boolean(mergedOther[p.apiKeyEnv]));
  if (available) {
  message += ` Tip: profile '${available.name}' has its credential configured. Run 'pi-worker doctor --profile ${available.name}' to use it, or set 'defaultProfile' in ~/.config/pi-worker/config.json.`;
  }
  }
  throw new WorkerError('DOCTOR_FAILED', message);
  }
  ...
  // Environment detection (non-fatal): enumerate every known worker CLI on this
  // machine and how many models each exposes, so the user sees at a glance what
  // is dispatchable. Failures here never block doctor — they only annotate.
  let environment = null;
  try {
    const reg = await detectEnvironment({ paths, env });
    environment = reg.adapters.map((a) => ({
      cli: a.name,
  ...
  return { status: 'ready', profile: profile.name, provider: profile.provider, model: profile.model, checks, environment };
}
