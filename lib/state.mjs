...
export function transition(state, nextStatus, patch = {}, reason = null) {
  ...
  // transition to non- failed clear at status failure/interruption history fields,avoid dashboard/inspect displays stale failure info.
  // transition to non- revising clear at status revisionRecovery(this recovery done,no longer relevant).
  const clearFields = {};
  if (nextStatus !== 'failed') { clearFields.failure = undefined; clearFields.interruption = undefined; }
  // revisionRecovery is one-time event record,not every transition clear at(retained as audit history)
  return {
  ...state,
  ...clearFields,
  ...patch,
...
export function recoverTransition(state, nextStatus, patch = {}, reason = 'recovered') {
  ...
  const at = new Date().toISOString();
  // clear at recovery failure/interruption(transition to non- failed status)
  const clearFields = {};
  if (nextStatus !== 'failed') { clearFields.failure = undefined; clearFields.interruption = undefined; }
  return {
  ...state,
