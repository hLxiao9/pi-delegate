...
export function transition(state, nextStatus, patch = {}, reason = null) {
  ...
  // 转到非 failed 状态时清除 failure/interruption 历史字段,避免 dashboard/inspect 展示过期的失败信息。
  // 转到非 revising 状态时清除 revisionRecovery(本次恢复已完成,不再相关)。
  const clearFields = {};
  if (nextStatus !== 'failed') { clearFields.failure = undefined; clearFields.interruption = undefined; }
  // revisionRecovery 是一次性事件记录,不在每次 transition 时清除(保留为审计历史)
  return {
    ...state,
    ...clearFields,
    ...patch,
...
export function recoverTransition(state, nextStatus, patch = {}, reason = 'recovered') {
  ...
  const at = new Date().toISOString();
  // 恢复时清除 failure/interruption(转到非 failed 状态)
  const clearFields = {};
  if (nextStatus !== 'failed') { clearFields.failure = undefined; clearFields.interruption = undefined; }
  return {
    ...state,
