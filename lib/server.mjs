...
export function createRequestHandler(paths, options = {}) {
  return async (req, res) => {
    ...
    try {
      ...
      if (env.PI_WORKER_DEBUG) process.stderr.write(`[pi-worker serve] internal error: ${error?.message ?? error}\n`);
...
function openBrowser(url) {
  ...
  try {
    if (platform === 'win32') {
      // 'start' is a CMD builtin, not an executable — spawn('start', ...)
      // fails with ENOENT on Windows. Use 'cmd /c start "" <url>' where ""
      // is the window-title placeholder required by the start command.
...
export async function serveCommand(options = {}, runtime = {}) {
  ...
  await new Promise((resolve, reject) => {
    ...
    // Bind to loopback only - the dashboard is a local dev tool, not a service
    // to expose to the network. Prevents other machines on the same LAN (or the
    // internet if the host has a public IP) from reading /api/runs.
