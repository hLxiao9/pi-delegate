...
// M5: high-risk uses independent error code HIGH_RISK_BLOCKED
...
// M3: explicit profile throws when not exists rather than silent fallback
...
// M5: high-risk task selectProfileForTask return directly null
...
// H5: maxConcurrentRuns range validation
...
// H3: metrics in legacy run(missing parentUsageStart)does not crash when
test('buildMetrics handles legacy run without parentUsageStart', async () => {
  ...
  // not pass parentUsageStart(legacy)
...
// H4: buildCohort for malformed metrics(missing quality/counterfactual)does not crash
test('buildCohort tolerates malformed metrics', async () => {
  ...
  // missing quality and counterfactual field
...
// M1: when all run all do not have saving rate data(e.g., using Kimi/Trae adapter None usage)when,recommend no-usage-data-available
test('buildCohort recommends no-usage-data-available when all runs lack saving rate', async () => {
  ...
  // has quality but counterfactual.estimatedCreditSavingRate missing(non-streaming adapter scenario)
...
// L5: runId contains .. or .lock rejected
